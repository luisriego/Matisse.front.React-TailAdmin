...
const UserProfile: React.FC = () => {
  ...
  const handleUserUpdate = async () => {
    ...
    if (token) {
      try {
        const decodedToken = jwtDecode<DecodedToken>(token);
        await fetchUserProfile(decodedToken.id, token);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6 bg-white dark:bg-white/[.03]">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:gap-7 2xl:gap-x-32">
                <div className="flex items-center gap-4 col-span-full">
                  <button
                    ...
                    className="overflow-hidden rounded-full border border-gray-200 dark:border-gray-800 flex items-center justify-center cursor-pointer"
                  >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                      {user.roles.join(', ') || 'Nenhum papel atribuído'}
                    </p>
                  </div>
                </div>
                <div>
              ...
              {user.residentUnit ? (
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:gap-7 2xl:gap-x-32">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-800 dark:text-white/90">
                      {user.residentUnit.idealFraction}
                    </p>
                  </div>
                  <div>
                    <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Status</p>
                    <p className="text-sm font-medium text-gray-800 dark:text-white/90">
                      {user.residentUnit.isActive ? 'Ativo' : 'Inativo'}
                    </p>
                  </div>
                  <div>
                    <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">Criado em</p>
                    <p className="text-sm font-medium text-gray-800 dark:text-white/90">
                      {formatDate(user.residentUnit.createdAt)}
                    </p>
                  </div>
      ...
      <EditUnitModal
        ...
        unit={user?.residentUnit}
        onUnitUpdate={handleUserUpdate}
      />
